...
$apiGovernance = Read-SourceText 'docs/quality/COMPONENT_API_GOVERNANCE_2_1.md'
...
Add-Gate 'Technical planning' 'Component API governance remains green' `
  ($(if (Test-AllTextContains $apiGovernance @('Component API Governance', 'Component roles mapped', 'Canonical property checks ready', 'Forbidden visible properties present: 0', 'Gates needing review: 0', 'TLazRibbon.BackstageView', 'TLazRibbonSkinManager')) { 'Ready' } else { 'Review' })) `
  'The component API governance report maps the effective Object Inspector surface to the documented composition model.' `
  'Regenerate the governance report after adding, hiding, renaming or removing package-facing component properties.'
...
$out.Add('This report turns "professional and adoption-ready" into repeatable checks. It complements the API freeze and component API governance reports by looking at the full package experience: trust, onboarding, Skin Editor workflow, demos and release automation.')
$out.Add('')
