...
type // Forward declaration for XML nodes
     ...
     // Data contained in the node
       FData : array of TLazRibbonXMLNode;
     ...
     // Getter dla liczby danych zawartych w wêle
       function GetCount : integer;
     // Getter dla danych zawartych w wêle
       function GetData(index : integer) : TLazRibbonXMLNode;
     public
     // Konstruktor
       constructor create;
     // Destruktor
     ...
     // Dodaje dane
       procedure Add(AData : TLazRibbonXMLNode);
     // Usuwa dane z listy (nie zwalnia!)
       procedure Remove(AData : TLazRibbonXMLNode);
     // Usuwa dane o zadanym indeksie (nie zwalnia!)
       procedure Delete(index : integer);
     // Usuwa wszystkie dane
       procedure Clear;

       property Data[index : integer] : TLazRibbonXMLNode read GetData;
     ...
     TLazRibbonXMLParameter = class(TObject)
     private
     // Nazwa parametru
       FName,
     // Wartoæ parametru
     ...
     // Konstruktor pozwalaj¹cy nadaæ pocz¹tkowe wartoci parametrowi
     ...
     // Lista parametrów
     TLazRibbonXMLParameters = class(TObject)
     private
     ...
     // Getter dla w³asnoci ParamByName (szuka parametru po jego nazwie)
       function GetParamByName(index : string; autocreate : boolean) : TLazRibbonXMLParameter;
     // Getter dla w³asnoci ParamByIndex (zwraca i-ty parametr)
       function GetParamByIndex(index : integer) : TLazRibbonXMLParameter;
     ...
     // Dodaje parametr na listê
       procedure Add(AParameter : TLazRibbonXMLParameter);
     // Wstawia parametr na listê na zadane miejsce
       procedure Insert( AIndex : integer; AParameter : TLazRibbonXMLParameter);
     // Usuwa parametr o podanym indeksie z listy
       procedure Delete(index : integer);
     // Usuwa zadany parametr z listy
       procedure Remove(AParameter : TLazRibbonXMLParameter);
     // Zwraca indeks zadanego parametru
       function IndexOf(AParameter : TLazRibbonXMLParameter) : integer;
     // Czyci listê parametrów
       procedure Clear;

       property ParamByName[index : string; autocreate : boolean] : TLazRibbonXMLParameter read GetParamByName; default;
       property ParamByIndex[index : integer] : TLazRibbonXMLParameter read GetParamByIndex;

       property Count : integer read GetCount;
     end;

     TLazRibbonBaseXmlNode = class;

     // Bazowa klasa dla ga³êzi XMLowych, zapewniaj¹ca przechowywanie, operacje
     // i wyszukiwanie podga³êzi.
     TLazRibbonBaseXmlNode = class(TObject)
     private
       FList : TObjectList;
       FTree : TBinaryTreeNode;
       FParent : TLazRibbonBaseXmlNode;
     protected
     // *** Operacje na drzewie AVL ***
     // Dodaje do drzewa ga³¹ z zadan¹ TLazRibbonXMLNode
       procedure TreeAdd(ANode : TLazRibbonXMLNode);
     // Usuwa z drzewa ga³¹ z zadan¹ TLazRibbonXMLNode
       procedure TreeDelete(ANode : TLazRibbonXMLNode);
     // Szuka ga³êzi drzewa
       function TreeFind(ANode : TLazRibbonXMLNode) : TBinaryTreeNode;
     ...
     // obróconego
       ...
       function GetNodeByIndex(index : integer) : TLazRibbonXMLNode;
       function GetNodeByName(index : string; autocreate : boolean) : TLazRibbonXMLNode;
       function GetCount : integer;
     public
     // Konstruktor
     ...
     // Dodaje podga³¹ i umieszcza w odpowiednim miejscu w drzewie
       procedure Add(ANode : TLazRibbonXMLNode);
     // Wstawia podga³¹ w podane miejsce (na drzewie ma to taki sam efekt
     // jak dodanie)
       procedure Insert(AIndex : integer; ANode : TLazRibbonXMLNode);
     // Usuwa podga³¹ z listy i z drzewa, a nastêpnie zwalnia pamiêæ
       procedure Delete(AIndex : integer);
     // Usuwa podga³¹ z listy i z drzewa, a nastêpnie zwalnia pamiêæ
       procedure Remove(ANode : TLazRibbonXMLNode);
     // Zwraca indeks podga³êzi
       function IndexOf(ANode : TLazRibbonXMLNode) : integer;
     // Usuwa wszystkie podga³êzie
       procedure Clear; virtual;

     // Metoda powinna zostaæ wywo³ana przed zmian¹ nazwy przez jedn¹ z podga³êzi
       procedure BeforeChildChangeName(AChild : TLazRibbonXmlNode);
     // Metoda powinna zostaæ wywo³ana po zmianie nazwy przez jedn¹ z podga³êzi
       procedure AfterChildChangeName(AChild : TLazRibbonXMLNode);

       property NodeByIndex[index : integer] : TLazRibbonXMLNode read GetNodeByIndex;
       property NodeByName[index : string; autocreate : boolean] : TLazRibbonXMLNode read GetNodeByName; default;
       property Count : integer read GetCount;
       property Parent : TLazRibbonBaseXmlNode read FParent write FParent;
     end;

     // Ga³¹ XMLa. Dziêki temu, ¿e dziedziczymy po TLazRibbonBaseXMLNode mamy
     // zapewnion¹ obs³ugê podga³êzi, trzeba tylko dodaæ parametry, nazwê i
     // tekst.
     TLazRibbonXMLNode = class(TLazRibbonBaseXMLNode)
     private
     // Nazwa ga³êzi
       FName : string;
     // Tekst ga³êzi
       FText : string;
     // Parametry ga³êzi
       FParameters : TLazRibbonXMLParameters;
     // Rodzaj ga³êzi
       FNodeType : TXMLNodeType;
     protected
     // Setter dla w³asnoci name (przed i po zmianie nazwy trzeba poinformowaæ
     ...
     // Destruktor
       destructor Destroy; override;
     // Czyci ga³¹ (tekst, parametry, podga³êzie)
       ...
       property Parameters : TLazRibbonXMLParameters read FParameters;
       property NodeType : TXMLNodeType read FNodeType;
     end;

     // Dziêki temu, ¿e dziedziczymy po TLazRibbonBaseXMLNode, mamy zapewnion¹ obs³ugê
     // podga³êzi
     TLazRibbonXMLParser = class(TLazRibbonBaseXMLNode)
     private
     protected
     public
     // Konstruktor
...
function TBinaryTreeNode.GetData(index : integer) : TLazRibbonXMLNode;
...
// Przeliczamy ponownie wysokoæ poddrzewa
RefreshSubtreeSize;
end;

procedure TBinaryTreeNode.Add(AData : TLazRibbonXMLNode);
begin
 {$B-}
...
procedure TBinaryTreeNode.Remove(AData : TLazRibbonXMLNode);
...
{ TLazRibbonXMLParameter }

constructor TLazRibbonXMLParameter.create;
begin
inherited create;
FName:='';
FValue:='';
...
constructor TLazRibbonXMLParameter.create(AName, AValue: string);
begin
inherited create;
FName:=AName;
FValue:=AValue;
...
destructor TLazRibbonXMLParameter.destroy;
begin
inherited destroy;
end;

function TLazRibbonXMLParameter.GetValueAsBoolean: boolean;
begin
...
function TLazRibbonXMLParameter.GetValueAsColor: TColor;
...
function TLazRibbonXMLParameter.GetValueAsExtended: extended;
begin
try
...
function TLazRibbonXMLParameter.GetValueAsInteger: integer;
begin
try
result:=StrToInt(FValue);
except
...
procedure TLazRibbonXMLParameter.SetValueAsBoolean(AValue: boolean);
begin
if AValue then FValue:='True' else FValue:='False';
end;

procedure TLazRibbonXMLParameter.SetValueAsColor(AValue: TColor);
begin
FValue:=IntToStr(AValue);
end;

procedure TLazRibbonXMLParameter.SetValueAsExtended(AValue: extended);
begin
FValue:=FloatToStr(AValue);
end;

procedure TLazRibbonXMLParameter.SetValueAsInteger(AValue: integer);
begin
FValue:=IntToStr(AValue);
end;

{ TLazRibbonXMLParameters }

procedure TLazRibbonXMLParameters.Add(AParameter: TLazRibbonXMLParameter);
begin
FList.add(AParameter);
end;

procedure TLazRibbonXMLParameters.Insert(AIndex : integer; AParameter : TLazRibbonXMLParameter);
...
procedure TLazRibbonXMLParameters.Clear;
begin
FList.clear;
end;

constructor TLazRibbonXMLParameters.create;
begin
inherited create;
...
procedure TLazRibbonXMLParameters.Delete(index: integer);
begin
...
procedure TLazRibbonXMLParameters.Remove(AParameter : TLazRibbonXMLParameter);

begin
FList.Remove(AParameter);
end;

destructor TLazRibbonXMLParameters.destroy;
begin
FList.Free;
inherited destroy;
end;

function TLazRibbonXMLParameters.GetCount: integer;
begin
result:=FList.count;
end;

function TLazRibbonXMLParameters.GetParamByIndex(index: integer): TLazRibbonXMLParameter;
begin
if (index<0) or (index>Flist.count-1) then
   raise exception.create('Invalid item index.');

result:=TLazRibbonXMLParameter(FList[index]);
end;

function TLazRibbonXMLParameters.GetParamByName(index: string;
  autocreate: boolean): TLazRibbonXMLParameter;

var i : integer;
    AParameter : TLazRibbonXMLParameter;

begin
// Szukamy elementu
i:=0;
while (i<=FList.count-1) and (uppercase(TLazRibbonXMLParameter(FList[i]).Name)<>uppercase(index)) do inc(i);

if i<=FList.count-1 then
   result:=TLazRibbonXMLParameter(FList[i]) else
   begin
   if autocreate then
      begin
      AParameter:=TLazRibbonXMLParameter.create(index,'');
      FList.add(AParameter);
      result:=AParameter;
      end else
          result:=nil;
...
function TLazRibbonXMLParameters.IndexOf(AParameter: TLazRibbonXMLParameter): integer;
begin
result:=FList.IndexOf(AParameter);
end;

{ TLazRibbonBaseXMLNode }

procedure TLazRibbonBaseXMLNode.TreeAdd(ANode : TLazRibbonXMLNode);
   ...
   // Nie ma potrzeby balansowania drzewa
       ...
       if Tree<>nil then
          ...
          // dodana ¿adna ga³¹
              ...
              // Zosta³a dodana nowa ga³¹, wiêc balansujemy drzewo (o ile jest
              // taka potrzeba)
              self.Ballance(Tree);
              end;
       end;
...
procedure TLazRibbonBaseXMLNode.TreeDelete(ANode : TLazRibbonXMLNode);
  ...
  begin
  // Najpierw sprawdzamy, czy bêdziemy usuwaæ korzeñ. Jeli tak, po usuniêciu
  ...
  // 1. Jeli ga³¹ ta przechowuje wiêcej ni¿ tylko ten element, to usuwamy go
  ...
  // 2. Jeli jest to liæ, po prostu usuwamy go.
  if (DelNode.Left=nil) and (DelNode.Right=nil) then
     ...
     // Ga³¹ automatycznie odpina wszystkie swoje podga³êzie, ale zak³adamy
     // tu, ¿e jest to liæ.
     DelNode.free;
  ...
  if (DelNode.Left=nil) xor (DelNode.Right=nil) then
     ...
     if DelParent=nil then
        ...
        if DelNode.Left<>nil then
           begin
           FTree:=DelNode.Left;
           // Mechanizmy drzewa odepn¹ automatycznie ga³¹ od DelNode, dziêki
  ...
  // 4. Zamieniamy zawartoæ "usuwanego" poddrzewa z jego nastêpnikiem, który
...
function TLazRibbonBaseXMLNode.TreeFind(ANode : TLazRibbonXMLNode) : TBinaryTreeNode;
...
procedure TLazRibbonBaseXMLNode.Ballance(Leaf : TBinaryTreeNode);
...
function TLazRibbonBaseXMLNode.RotateLeft(Root : TBinaryTreeNode) : TBinaryTreeNode;
...
function TLazRibbonBaseXMLNode.RotateRight(Root : TBinaryTreeNode) : TBinaryTreeNode;
...
function TLazRibbonBaseXMLNode.GetNodeByIndex(index : integer) : TLazRibbonXMLNode;

begin
if (index<0) or (index>FList.count-1) then
   raise exception.create('Invalid index!');

result:=TLazRibbonXMLNode(FList[index]);
end;

function TLazRibbonBaseXMLNode.GetNodeByName(index : string; autocreate : boolean) : TLazRibbonXMLNode;

var Tree : TBinaryTreeNode;
    XmlNode : TLazRibbonXMLNode;
...
if Tree<>nil then result:=Tree.Data[0] else
   ...
   if not(autocreate) then
      result:=nil else
      begin
      XmlNode:=TLazRibbonXMLNode.create(index,xntNormal);
      TreeAdd(XmlNode);
      FList.add(XmlNode);
      result:=XmlNode;
      end;
...
function TLazRibbonBaseXMLNode.GetCount : integer;

begin
result:=FList.Count;
end;

constructor TLazRibbonBaseXMLNode.create;
...
destructor TLazRibbonBaseXMLNode.destroy;
...
procedure TLazRibbonBaseXMLNode.Add(ANode : TLazRibbonXMLNode);
begin
  if ANode = self then
...
procedure TLazRibbonBaseXMLNode.Insert(AIndex : integer; ANode : TLazRibbonXMLNode);
...
procedure TLazRibbonBaseXMLNode.Delete(AIndex : integer);

begin
if (AIndex<0) or (AIndex>FList.count-1) then
   raise exception.create('Invalid index');

TreeDelete(TLazRibbonXMLNode(FList[AIndex]));

// Poniewa¿ FList.OwnsObjects, automatycznie zwolni usuwany element.
FList.delete(AIndex);
end;

procedure TLazRibbonBaseXMLNode.Remove(ANode : TLazRibbonXMLNode);
...
function TLazRibbonBaseXMLNode.IndexOf(ANode : TLazRibbonXMLNode) : integer;

begin
result:=FList.IndexOf(ANode);
end;

procedure TLazRibbonBaseXMLNode.Clear;
...
procedure TLazRibbonBaseXMLNode.BeforeChildChangeName(AChild : TLazRibbonXmlNode);

begin
TreeDelete(AChild);
end;

procedure TLazRibbonBaseXMLNode.AfterChildChangeName(AChild : TLazRibbonXMLNode);
...
procedure TLazRibbonXMLNode.SetName(Value : string);
...
function TLazRibbonXMLNode.GetTextAsInteger : integer;
...
procedure TLazRibbonXMLNode.SetTextAsInteger(value : integer);

begin
FText:=IntToStr(value);
end;

function TLazRibbonXMLNode.GetTextAsExtended : extended;
...
procedure TLazRibbonXMLNode.SetTextAsExtended(value : extended);

begin
FText:=FloatToStr(value);
end;

function TLazRibbonXMLNode.GetTextAsColor : TColor;
...
procedure TLazRibbonXMLNode.SetTextAsColor(value : TColor);

begin
FText:=IntToStr(value);
end;

function TLazRibbonXMLNode.GetTextAsBoolean : boolean;
...
procedure TLazRibbonXMLNode.SetTextAsBoolean(value : boolean);

begin
if value then FText:='True' else FText:='False';
end;

constructor TLazRibbonXMLNode.create(AName : string; ANodeType : TXMLNodeType);
...
FText:='';
FNodeType:=ANodeType;
FParameters:=TLazRibbonXMLParameters.create;
end;

destructor TLazRibbonXMLNode.destroy;
...
procedure TLazRibbonXMLNode.Clear;
...
constructor TLazRibbonXMLParser.create;

begin
inherited create;
end;

destructor TLazRibbonXMLParser.destroy;

begin
inherited destroy;
end;

procedure TLazRibbonXMLParser.Parse(input : PChar);
...
var // Stos przetwarzanych ga³êzi (niejawna rekurencja)
  ...
  // Funkcja inkrementuje wskanik wejcia, pilnuj¹c jednoczenie, by uaktualniæ
  // pozycjê w pliku
    ...
    for i:=1 to length(s) do
        ...
        if s[i] in ['0'..'9'] then result:=result+ord(s[i])-ord('0') else
           if UpCase(s[i]) in ['A'..'F'] then result:=result+ord(s[i])-ord('A')+10 else
              raise exception.create('Invalid hexadecimal number!');
        end;
    end;
  ...
  while (input^<>TextDelimiter) or ((input^='<') and (StrLComp(input,'<![CDATA[',9)=0)) do
        ...
        // Jeli napotkalimy nawias k¹towy, mo¿e to byæ sekcja CDATA
...
try
  ...
  case CurrentOperation of
       poNodes : begin
                 ...
                 // Wejcie mo¿e siê tu koñczyæ tylko wtedy, gdy jestemy
       ...
       poControlInterior : begin
                           ...
                           if CurrentOperation=poTagInterior then
                              begin
                              Node:=TLazRibbonXMLNode.create('',xntNormal);

                              // Pomijamy znak otwarcia taga
                              increment(input);
                              end else
                                  begin
                                  Node:=TLazRibbonXMLNode.create('',xntControl);
                           ...
                           // Pêtla koñczy siê, gdy na wejciu nie ma ju¿
                           ...
                           if CurrentOperation=poControlInterior then
                              ...
                              if NodeStack.count>0 then
                                 TLazRibbonXMLNode(NodeStack.Peek).Add(Node) else
                                 Self.Add(Node);
                           ...
                           // Jeli co pójdzie nie tak, ga³¹ wisi w pamiêci i
       ...
       poCommentInterior : begin
                           ...
                           Node:=TLazRibbonXMLNode.create('',xntComment);
       ...
       poClosingInterior : begin
                           ...
                           if uppercase(s)<>uppercase(TLazRibbonXMLNode(NodeStack.Peek).Name) then
                              raise exception.create('Error in XML syntax (line '+IntToStr(ParseLine)+', character '+IntToStr(ParseChar)+'): The closing tag (' + s + ') does not match the opening tag ('+TLazRibbonXMLNode(NodeStack.Peek).Name+') !');

                           // Wszystko OK, zdejmujemy tag ze stosu i dodajemy go do taga pod nim
                           Node:=TLazRibbonXMLNode(NodeStack.Pop);
       ...
       poTagText : begin
                   ...
                   TLazRibbonXMLNode(NodeStack.Peek).Text:=s;

                   CurrentOperation:=poNodes;
                   end;
  end;

  // Jeli na stosie pozosta³y jakie ga³êzie - oznacza to b³¹d (nie zosta³y
  // domkniête)

  if NodeStack.Count>0 then
     raise exception.create('Error in XML syntax (line '+IntToStr(ParseLine)+', character '+IntToStr(ParseChar)+'): Unexpected end of file (there are unclosed tags, the first of them is '+TLazRibbonXMLNode(NodeStack.Peek).Name+')');

  // Wszystko w porz¹dku, XML zosta³ wczytany.
finally

  // Czycimy nie przetworzone ga³êzie
...
function TLazRibbonXMLParser.Generate(UseFormatting : boolean) : string;

  function InternalGenerate(RootNode : TLazRibbonXMLNode; indent : integer; UseFormatting : boolean) : string;
  ...
  var i : integer;
    ...
    if CheckWhitespace then
       begin
       // Jeli pierwszy znak jest bia³y, zamieñ go na encjê
       if s[1]=#32 then
          begin
          System.delete(s,1,1);
          prefix:='&#32;';
       ...
       // Jeli ostatni znak jest bia³y, zamieñ go na encjê
       if length(s)>0 then
          begin
  ...
  if RootNode=nil then
     ...
     if FList.count>0 then
        for i:=0 to FList.count-1 do
            result:=result+InternalGenerate(TLazRibbonXMLNode(FList[i]),0,UseFormatting);
     end else
         begin
...
procedure TLazRibbonXMLParser.LoadFromFile(AFile : string);
var
  sl : TStringList;
begin
...
procedure TLazRibbonXMLParser.SaveToFile(AFile : string; UseFormatting : boolean);
var
  sl: TStringList;
begin
  sl:=TStringList.create;
...
procedure TLazRibbonXMLParser.LoadFromStream(AStream : TStream);
var
  sl: TStringList;
begin
  sl:=TStringList.create;
...
procedure TLazRibbonXMLParser.SaveToStream(AStream : TStream; UseFormatting : boolean);
var
  sl: TStringList;
begin
  sl:=TStringList.create;
