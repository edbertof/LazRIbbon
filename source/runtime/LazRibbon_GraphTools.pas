...
*******************************************************************************)
  ...
  public
    ...
    class function GetContrastColor(AColor, ADarkColor, ABrightColor: TColor): TColor;
  end;
...
begin
  c := TRgbColor(ColorToRGB(AColor));
  gray := Trunc(0.222 * c.R + 0.707 * c.G + 0.071 * c.B);
...
class function TColorTools.GetContrastColor(AColor, ADarkColor, ABrightColor: TColor): TColor;
var
  c: TRgbColor;
begin
  c := TRgbColor(ColorToRGB(AColor));
  if (Integer(c.R) + c.G + c.B) > 3*128 then
    Result := ADarkColor
  else
    Result := ABrightColor;
end;
