...
uses
  ...
  LazRibbon_GraphTools, LazRibbon_Math;
...
type
  ...
  TLazRibbonButtonState = (bsIdle,
    bsBtnHotTrack, bsBtnPressed, bsDropdownHottrack, bsDropdownPressed);

  TLazRibbonCheckboxStyle = (cbsCheckbox, cbsRadioButton, cbsToggleSwitch);
  ...
  public
    ...
    class procedure DrawCheckbox(ACanvas: TCanvas;
                                 ...
                                 AButtonState: TLazRibbonButtonState;
                                 AStyle: TLazRibbonCheckboxStyle); overload;
    class procedure DrawCheckbox(ACanvas: TCanvas;
                                 x,y: Integer;
                                 AState: TCheckboxState;
                                 AButtonState: TLazRibbonButtonState;
                                 AStyle: TLazRibbonCheckboxStyle;
                                 ClipRect: T2DIntRect); overload;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SrcPtr := {%H-}pointer({%H-}PtrUInt(SrcLine) + SrcRect.Left*3);
        DstPtr := {%H-}pointer({%H-}PtrUInt(DstLine) + 3*(SrcRect.Left + Offset.x));
  ...
  begin
    ...
    begin
      ...
      begin
        SrcLine := SrcImg.GetDataLineStart(y);
        DstLine := DestImg.GetDataLineStart(y+Offset.y);
        SrcPtr := {%H-}pointer({%H-}PtrUInt(SrcLine) + 3*SrcRect.Left);
        DstPtr := {%H-}pointer({%H-}PtrUInt(DstLine) + 3*(SrcRect.Left + Offset.x));
        for x := SrcRect.Left to SrcRect.Right do
        begin
          {$ifdef EnhancedRecordSupport}
...
{$ENDIF}
  ...
  begin
    ...
    begin
      SrcLine := SrcImg.GetDataLineStart(y);
      DstLine := DestImg.GetDataLineStart(y+Offset.y);
      Move({%H-}pointer({%H-}PtrUInt(SrcLine) + 3*ClippedSrcRect.Left)^,
           {%H-}pointer({%H-}PtrUInt(DstLine) + 3*(ClippedSrcRect.Left + Offset.x))^,
...
begin
 if (AMask.PixelFormat <> pf8bit) then
   raise Exception.Create('TLazRibbonGUITools.CopyRoundCorner: Only 8-bit masks are accepted!');
  ...
  begin
    ...
    begin
      SrcLine := SrcImg.GetDataLineStart(y);
      SrcLine := {%H-}pointer({%H-}PtrUInt(SrcLine) + 3 * ClippedSrcRect.left);
      MaskLine := MaskImg.GetDataLineStart(y);
      MaskLine := {%H-}pointer({%H-}PtrUInt(MaskLine) + ClippedSrcRect.left);
      DstLine := DestImg.GetDataLineStart(y+Offset.y);
      DstLine := {%H-}pointer({%H-}PtrUInt(DstLine) + 3 * (ClippedSrcRect.left + Offset.x));
      for i := 0 to ClippedSrcRect.Width - 1 do
      begin
        if PByte(MaskLine)^ < 128 then
          Move(SrcLine^, DstLine^, 3);
        SrcLine := {%H-}pointer({%H-}PtrUInt(SrcLine)+3);
        DstLine := {%H-}pointer({%H-}PtrUInt(DstLine)+3);
        MaskLine := {%H-}pointer({%H-}PtrUInt(MaskLine)+1);
      end;
    end;
    ABitmap.LoadFromIntfImage(DestImg);
    DestImg.Destroy;
...
begin
  if AMask.PixelFormat<>pf8bit then
    raise Exception.Create('TLazRibbonGUITools.CopyMaskRectangle: Only 8-bit masks are accepted!');
  ...
  then
    raise Exception.Create('TLazRibbonGUITools.CopyMaskRectangle: The mask has incorrect dimensions!');
...
class procedure TGUITools.DrawCheckbox(ACanvas:TCanvas; x,y: Integer;
  AState: TCheckboxState; AButtonState:TLazRibbonButtonState;
  AStyle: TLazRibbonCheckboxStyle; ClipRect:T2DIntRect);
var
  UseOrgClipRgn: Boolean;
  OrgRgn: HRGN;
  ClipRgn: HRGN;
...
const
  UNTHEMED_FLAGS: array [TLazRibbonCheckboxStyle, TCheckboxState] of Integer = (
  ...
  THEMED_FLAGS: array [TLazRibbonCheckboxStyle, TCheckboxState, TLazRibbonButtonState] of TThemedButton = (
    ( // CheckBox
    ...
    ( // ToggleSwitch (not painted by this method...)
...
begin
  if ThemeServices.ThemesEnabled then begin
    ...
    sz := ThemeServices.GetDetailSizeForPPI(te, ScreenInfo.PixelsPerInchX);
