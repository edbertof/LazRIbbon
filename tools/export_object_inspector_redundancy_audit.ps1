...
$out.Add('## Summary')
$out.Add('')
$out.Add(('- Effective Object Inspector properties scanned: {0}' -f $properties.Count))
...
$out.Add('| ' + (ConvertTo-InlineCode 'Appearance') + ' | Accept only where the component owns a complete appearance model. The top-level Ribbon uses ' + (ConvertTo-InlineCode 'SkinManager') + ' and ' + (ConvertTo-InlineCode 'AppearanceSource') + ' as the visible design-time workflow. |')
