...
- `tools/check_project_consistency.ps1 -ExpectedVersion 2.1.19` passes.
...
The Skin Editor Sample demo now ships with a valid `MeuSkin.lazskin` metadata
...
The Skin Editor validation step now has a decision-oriented action panel. The
former recommended-actions memo starts with an explicit save-readiness status,
...
`TLazRibbonBackstageView` now has a dedicated component editor for the normal design-time composition workflow. The developer can add page, command and separator entries directly and link the BackStage to the Ribbon on the same form. The linking command uses the existing canonical properties, carries over the Ribbon's `SkinManager` and applies Office-like overlay defaults. No new published component API was added.

## 2.1.19 Snapshot

- `qapBeforeTabs` now follows its literal layout order: QAT, Application/Arquivo
  button and then the normal Ribbon tabs.
- `qapTitleBar` uses a visible `TLazRibbonForm` custom title bar when available
  and falls back to the before-tabs layout otherwise, preventing silent command
  disappearance on ordinary forms or when custom chrome is disabled.
- The effective placement is shared by metrics, painting, mouse interaction and
  KeyTips while the developer's configured `Position` remains unchanged.

## 2.1.18 Snapshot

- Recent-file selection now follows the complete Office-like workflow: invoke
  `OnItemClick`, open the selected document in application code and close the
  containing BackStage.
- `TLazRibbonBackstageRecentList.CloseBackstageOnClick` defaults to `True` and
  provides an explicit opt-out for specialized workflows.
- The parent lookup supports lists nested inside ordinary BackStage page
  controls instead of requiring the list to be a direct child of the view.

## 2.1.17 Snapshot

BackStage navigation no longer changes icon size according to `Kind`.
`bbkPage` and `bbkCommand` use `Images`/`ImageIndex` by default, while
`LargeImageIndex` remains an explicit choice for either kind. Pages now expose
their own `ImageIndex`, inherited by the linked page item when that item does
not define an icon directly.
...
The advanced visual page now makes base differences actionable. The difference
summary includes a clickable list of changed Appearance properties; selecting an
item moves the main inspector to the matching property, and double-clicking opens
the property editor directly.
